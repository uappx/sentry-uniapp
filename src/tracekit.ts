...
// Updated to support relative paths (e.g., app-service.js) for Android App platform
const chrome = /^\s*at (?:(.*?) ?\()?((?:file|https?|blob|chrome-extension|native|eval|webpack|<anonymous>|[-a-z]+:|\/).*?|[^\s]+\.js)(?::(\d+))?(?::(\d+))?\)?\s*$/i;
